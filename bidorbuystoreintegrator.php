...
define('BOBSI_WOOCOMMERCE_CURRENCY_CONVERTER_PLUGIN_PHP_FILE', 'woocommerce-currency-converter/woocommerce-currency-converter.php');
define('BOBSI_WOOCOMMERCE_CURRENCY_CONVERTER_PLUGIN_FILE', WP_PLUGIN_DIR . '/' . BOBSI_WOOCOMMERCE_CURRENCY_CONVERTER_PLUGIN_PHP_FILE);
...
$dbSettings = array(
    bobsi\Db::SETTING_PREFIX => $wpdb->prefix,
...
function bobsi_plugin_activate() {
    ...
    if (!is_plugin_active(BOBSI_WOOCOMMERCE_PLUGIN_PHP_FILE)) {
        bobsi_exit_with_error(bobsi\Version::$name . ' requires <a href="http://www.woothemes.com/woocommerce/"
            ...
            . admin_url('plugin-install.php?tab=search&type=term&s=WooCommerce') . '" target="_blank">WooCommerce</a> first.');
    }

    if (
    !($wpdb->query(bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getQueries()->getInstallAuditTableQuery())
        && $wpdb->query(bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getQueries()->getInstallTradefeedTableQuery())
        && $wpdb->query(bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getQueries()->getInstallTradefeedDataTableQuery()))
    ) {
    ...
    update_option(bobsi\Settings::name, bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->serialize(true));
}
...
function bobsi_add_all_products_in_tradefeed_queue($update = false) {
    ...
    foreach ($productsIds as $page) {
        if (!$wpdb->query(bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getQueries()->getAddJobQueries($page, $productStatus))) {
            return false;
        }
    }
...
function bobsi_options() {
    ...
    if (isset($_POST['submit_options']) && $_POST['submit_options'] == 1) {
        ...
        if (!$prevent_saving) {
            ...
            $previousSettings = bobsi\StaticHolder::getBidorbuyStoreIntegrator()
...
//            $notes = array('The export criteria settings have been successfully changed. Please re-export your products.');
        }
    }
    ...
    foreach ($currencies as $currency) {
        $selected = ($currency == bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getCurrency()) ? 'selected="selected"' : '';
    ...
    $included_statuses = '<select id="bobsi-inc-statuses" class="bobsi-select" name="exportStatuses[]" multiple="multiple" size="9">';
    $excluded_statuses = '<select id="bobsi-exc-statuses" class="bobsi-select" name="excludeStatuses[]" multiple="multiple" size="9">';
    ...
    $excluded_categories = '<select id="bobsi-exc-categories" class="bobsi-categories-select" name="excludeCategories[]" multiple="multiple" size="9">';
    ...
    $zip_loaded = array_key_exists('zip', bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getCompressLibraryOptions());
    $export_link = BOBSI_PLUGIN_URL . 'export.php?t=' . bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getTokenExport();
    $download_link = BOBSI_PLUGIN_URL . 'download.php?t=' . bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getTokenDownload();
    $resetaudit_link = BOBSI_PLUGIN_URL . 'resetaudit.php?t=' . bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getTokenDownload();
    $phpInfo_link = BOBSI_PLUGIN_URL . 'version.php?t=' . bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getTokenDownload().'&phpinfo=y';
    ...
    $compress_libs = '';
    foreach (bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getCompressLibraryOptions() as $lib => $descr) {
        $selected = (bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getCompressLibrary() == $lib) ? 'selected="selected"' : '';
        $compress_libs .= '<option value="' . $lib . '" ' . $selected . '>' . $lib . '</option>';
    }
    $bobsi_default_quantity = bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getDefaultStockQuantity();
    $bobsi_email_notification_addresses = bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getEmailNotificationAddresses();
    $bobsi_enable_email_notification = (bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getEnableEmailNotifications()) ? 'checked="checked"' : '';
    $logging_levels = '';
    foreach (bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getLoggingLevelOptions() as $level) {
        $selected = (bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getLoggingLevel() == $level) ? 'selected="selected"' : '';
    ...
    $only_active_products = 1; //bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getExportActiveProducts();
...
function bobsi_admin_menu() {
    ...
    function bobsi_required_scripts() {
        wp_enqueue_script('bobsi_admin', plugins_url('vendor/com.extremeidea.bidorbuy/storeintegrator-core/assets/js/admin.js', __FILE__), array('jquery', 'jquery-tiptip', 'woocommerce_admin'));
...
function plugin_update()
{
    ...
    bobsi_add_all_products_in_tradefeed_queue(true);
    $query = "ALTER TABLE ".$wpdb->prefix.bobsi\Queries::TABLE_BOBSI_TRADEFEED." ADD `images` text AFTER `image_url`";
    $wpdb->query($query);
