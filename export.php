...
function &bobsi_build_export_product(&$product, $variations = array(), $categories = array()) {
    ...
    if ($product->is_on_sale()) {
        $exportedProduct[bobsi\Tradefeed::nameProductPrice] = bobsi_convert_price((double)$product->get_price_including_tax());

        //Is it real sale?
        if ($product->get_price() !== $product->regular_price) {
            $price = $product->get_price();
            $product->set_price($product->regular_price);
            $exportedProduct[bobsi\Tradefeed::nameProductMarketPrice] = bobsi_convert_price((double)$product->get_price_including_tax());
    ...
    //$product->id = $id; // It is 0 by default, besides $product->get_shipping_class() returns "slug" instead of "name"
    ...
    $exportedProduct[bobsi\Tradefeed::nameProductShippingClass] =
        ($product instanceof WC_Product_Variation) ? bobsi_get_shipping_class($product->variation_id, $product->parent->id) : bobsi_get_shipping_class($product->id);
    ...
    if ($product->width) {
        $exportedProduct[bobsi\Tradefeed::nameProductAttributes][bobsi\Tradefeed::nameProductAttrWidth] = number_format($product->width, 2, '.', '');
//        $exportedProduct[bobsi\Tradefeed::nameProductAttributes][bobsi\Tradefeed::nameProductAttrWidth] = intval($product->width);
    }

    if ($product->height) {
        $exportedProduct[bobsi\Tradefeed::nameProductAttributes][bobsi\Tradefeed::nameProductAttrHeight] = number_format($product->height, 2, '.', '');
//        $exportedProduct[bobsi\Tradefeed::nameProductAttributes][bobsi\Tradefeed::nameProductAttrHeight] = intval($product->height);
    }

    if ($product->length) {
        $exportedProduct[bobsi\Tradefeed::nameProductAttributes][bobsi\Tradefeed::nameProductAttrLength] = number_format($product->length, 2, '.', '');
//        $exportedProduct[bobsi\Tradefeed::nameProductAttributes][bobsi\Tradefeed::nameProductAttrLength] = intval($product->length);
    }

    if ($product->has_weight()) {
        $exportedProduct[bobsi\Tradefeed::nameProductAttributes][bobsi\Tradefeed::nameProductAttrShippingWeight] = number_format($product->get_weight(), 2, '.', '') . get_option('woocommerce_weight_unit', '');
//        $exportedProduct[bobsi\Tradefeed::nameProductAttributes][bobsi\Tradefeed::nameProductAttrWeight] = intval($product->get_weight());
    }

    $exportedProduct[bobsi\Tradefeed::nameProductAvailableQty] = bobsi_calc_product_quantity($product, bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getDefaultStockQuantity());
    ...
    $exportedProduct[bobsi\Tradefeed::nameProductCategory] = join(bobsi\Tradefeed::categoryNameDelimiter, $categorie_names);
...
function &bobsi_export_products($id, $available_variations = array()) {
    ...
    $exportVisibilities = bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getExportVisibilities();
    ...
    if (count(bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getExportStatuses()) == 0 OR !in_array($product->post->post_status, bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getExportStatuses())) {
        return $exportedProducts;
    }

    $allowedCategories = bobsi_get_export_categories_ids(bobsi\StaticHolder::getBidorbuyStoreIntegrator()->getSettings()->getExcludeCategories());
    ...
    if ($product instanceof WC_Product_Variable) {
        ...
        $attributes = $product instanceof WC_Product_Variation ? $product->parent->get_attributes() : $product->get_attributes();
        $attributes_variations = $product instanceof WC_Product_Variation ? $product->get_variation_attributes() : array();
        ...
        if (bobsi_calc_product_quantity($product, $defaultStockQuantity) > $exportQuantityMoreThan) {
                ...
                foreach ($available_variations_copy as &$variation) {
                    ...
                    if (preg_match('/^' . $mhash . '$/', $hash)) {
                        ...
                        if (intval($p[bobsi\Tradefeed::nameProductPrice]) > 0) {
                            ...
                            bobsi\StaticHolder::getBidorbuyStoreIntegrator()->logInfo('Product price <= 0, skipping, product id: ' . $product->id);
                        }
                    } else {
            ...
            bobsi\StaticHolder::getBidorbuyStoreIntegrator()->logInfo('QTY is not enough to export product id: ' . $id);
        }
...
//      if ((isset($attribute['is_visible']) && $attribute['is_visible'])) {
        $variations['attributes'][bobsi_attribute_label($attribute['name'])] = isset($attributes_variations['attribute_' . $key])
            ? $attributes_variations['attribute_' . $key]
...
$productStatus = isset($_POST[bobsi\Settings::paramProductStatus]) ? $_POST[bobsi\Settings::paramProductStatus] : false;
